...
class CategorySection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final placesWithImage =
        category.places.where((p) => p.hasValidCoverImage).take(10).toList();
    ...
    final summaryStyle = AppTheme.bodySmall(context).copyWith(
      color: AppTheme.darkGray,
      height: 1.3,
      fontSize: 13,
    );
    final summaryMaxHeight = _measureSummaryMaxHeight(
      context,
      placesWithImage,
      summaryStyle,
    );
    final listHeight =
        210.0 + (summaryMaxHeight > 0 ? 8.0 + summaryMaxHeight : 0.0);
  ...
  double _measureSummaryMaxHeight(
    ...
    for (final place in places) {
      if (place.summary.isEmpty) continue;
      final painter = TextPainter(
        text: TextSpan(text: place.summary, style: summaryStyle),
        maxLines: 3,
...
class CategorizedPlacesList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final validCategories = categories.where((c) {
      final placesWithImage =
          c.places.where((p) => p.hasValidCoverImage).length;
    ...
    return Column(
      ...
      children: [
        for (int i = 0; i < validCategories.length; i++) ...[
          ...
          if (i < validCategories.length - 1)
            const SizedBox(height: 12), // 减小分类间距
        ],
      ],
    );
  }
